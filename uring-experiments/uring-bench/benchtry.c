...
uint64_t pack(uint64_t a, uint8_t b)
{
  return  ((uint64_t)(a << 8)) | b;
}

uint64_t move(uint64_t a)
{
  return (a << 8);
}

uint8_t getdep(uint64_t a)
{
  return a & 0xFF;
}

// Returns 0 if no CQE was reaped or the start ts of the reaped request.
ts cq_poll(struct cqring* cq, struct sqring* sq, void* buf, size_t len, off_t off) {
  ...
  uint8_t dep = getdep(cqe->user_data);
  /*
  if (dep != 0)
    printf("dep is %d\n", dep);
  */
  if (dep <= DEPNO)
  {
    head++;
    *cq->head = head;
    sq_submit_io_dep(sq, buf, len, off, dep + 1);
    return 0;
  }
...
void init_io(struct iovec* vec, off_t off, struct io_uring_sqe* sqe) {
  ...
  sqe->len = 1;
  sqe->rw_flags = 0;
  sqe->user_data = pack(rdtsc(), 0); // XXX This isn't right -- should use last deadline?
  sqe->buf_index = 0;
}
...
void init_io_dep(struct iovec* vec, off_t off, struct io_uring_sqe* sqe, uint8_t dep) {
  sqe->opcode = IORING_OP_READV;
  sqe->flags = IOSQE_FIXED_FILE;
  sqe->ioprio = 0;
  sqe->fd = 0; // See io_uring_register; index into a "registered fd array".
  ...
  sqe->len = 1;
  sqe->rw_flags = 0;
  sqe->user_data = pack(rdtsc(), dep); // XXX This isn't right -- should use last deadline?
  //printf("%64x\n", sqe->user_data);
  sqe->buf_index = 0;
}

void sq_submit_io_dep(struct sqring* sq, void* buf, size_t len, off_t off, uint8_t dep) {
  uint32_t tail = *sq->tail;
  const uint32_t index = tail & *sq->mask;
  ...
  vec->iov_base = buf;
  vec->iov_len = len;
  init_io_dep(vec, off, &sq->sqes[index], dep);
    ...
    bool use_polling)
{
  ...
  size_t off = xorshift64() & off_mask & (~0xfff);
  while (start < all_done_ts) {
    // Reap ready CQEs eagerly because to attach end timestamps to them.
    ts cqe_ts;
   while (start < deadline) { // O_DIRECT needs block-aligned reads.
      if ((cqe_ts = cq_poll(&cq, &sq, buf, buf_size, off)) != 0) { // XXXXX Need to do wait until logic here too....
        ts end = rdtsc();
        completed++;

        warmup = warmup && (end < warmup_done_ts);
        if (!warmup) {
          samples[sample_count++] = (move(end) - cqe_ts);
